...
namespace TryHamburger
{
    public class Facade
    {
        ...
        public static async Task PopulateLatestNewsAsync(ObservableCollection<DataWrapper> latestNews, string sorting, int Type)
        {
        ...
        private static async Task<List<DataWrapper>> GetNewsDataWrapperAsync(string sortMode, int Type)
        {
            ...
            DataContractJsonSerializer serializer = new DataContractJsonSerializer(typeof(List<DataWrapper>));
            var ms = new MemoryStream(Encoding.UTF8.GetBytes(jsonMessage));

            var result = (List<DataWrapper>)serializer.ReadObject(ms);
            return result;
        }
